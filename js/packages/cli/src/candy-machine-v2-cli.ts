...
programCommand('get_all_mint_addresses').action(async (directory, cmd) => {
  ...
  const launchOptimistic = accountsByCreatorAddress.map(elem => (
    {
      mintAddress:new PublicKey(elem[0].mint).toBase58(),
      tokenName: elem[0].data.name
    } 
  ));
      ...
      chunks(Array.from(Array(keys.length).keys()), 500).map(
        async allIndexesInSlice => {
          for (let i = 0; i < allIndexesInSlice.length; i++) {
            ...
            const cacheItem = cacheContent.items[key];
            cacheItem.mintAddress = launchOptimistic.find(x=>x.tokenName==cacheItem.name).mintAddress;   
          }
        },
      ),
    );
